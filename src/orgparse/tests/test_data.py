...
def test_data(dataname):
    ...
    for (i, (node, kwds)) in enumerate(zip(root[1:], data)):
        for key in kwds:
            ...
            assert (
                kwds[key] == val
            ), 'check value of {0}-th node of key "{1}" from "{2}".\n\nParsed:\n{3}\n\nReal:\n{4}'.format(
                i, key, dataname, val, kwds[key]
            )
            assert type(kwds[key]) == type(
                val
            ), 'check type of {0}-th node of key "{1}" from "{2}".\n\nParsed:\n{3}\n\nReal:\n{4}'.format(
                i, key, dataname, type(val), type(kwds[key])
            )
